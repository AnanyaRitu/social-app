...
def follow(request):
    if request.method=='POST':
        ...
        if FollowerCount.objects.filter(follower=follower, user=user).first():
            delete_follower=FollowerCount.objects.create(follower=follower, user=user)
...
# user suggestion starts
    ...
    return render(request,'index.html', {'user_profile':user_profile,
          'posts':feed_list, 'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
